---
description: "Web backend boot support: environment layers, patches, diagnostics, and configuration preview."
...
The Web backend loads the `web` profile from `$DSH_HOME/profiles/web`. It combines the base and Web bundles with the profile's own `cordis.patch.yml` and reloads user patches while running. The profile library also supports explicit bundle lists and `patchReload: live | startup`; omitted reload policy defaults to `live`. Missing bundles and bundles without a patch declaration fail startup.
...
- **Channel-neutral library.** The package carries no loader hooks and no dev-mode surface; the [`dsh` app](../../../apps/server/README.md) owns its Node source-launch hook and consumes these helpers for the boot sequence, and built consumers use plain Node package resolution.
...
- [dsh app](../../../apps/server/README.md) — the `dsh` bin that consumes these helpers.
- [dsh-cmdline](../cmdline/README.md) — the launcher-to-app command-line handoff the bins use.
- [Profile bundles](../../bundle/README.md) — installable patch layers composed by the Web backend.
