...
describe('loadProfile', () => {
  ...
  it('auto-initializes only shipped templates and fails loud otherwise', () => {
    ...
    expect(Object.keys(PROFILE_TEMPLATES)).toEqual(['web'])
    for (const removed of ['headless', 'sdk', 'sdk-minimal', 'acp']) {
      expect(() => loadProfile('t', removed, anchor, home)).toThrow('does not exist')
    }
    try {
