---
description: "Web 后端启动支持：环境分层、补丁、诊断和配置预览。"
...
`dsh-app-boot` 加载环境分层，组合 Web profile 的 bundle 和补丁，并启动后端插件。它返回运行中的应用，或指出失败的插件及原因。Web 后端是受支持的应用入口；直接配置辅助函数用于内部测试。配置预览和实时补丁使用同一个 profile 解析器。
...
Web 后端从 `$DSH_HOME/profiles/web` 加载 `web` profile。它将基础 bundle 和 Web bundle 与 profile 自身的 `cordis.patch.yml` 组合，并在运行期间重新加载用户补丁。profile 库也支持显式 bundle 列表和 `patchReload: live | startup`；省略重载策略时默认为 `live`。缺少 bundle 或 bundle 未声明补丁文件时，启动失败。
...
- **与渠道无关的库。** 此包不包含 loader 钩子，也不提供开发模式接口；[`dsh` 应用](../../../apps/server/README.zh.md) 持有自己的 Node 源码启动钩子，并在启动序列中使用这些 helper，构建后的消费方则使用普通 Node 包解析。
...
- [dsh 应用](../../../apps/server/README.zh.md)——消费这些 helper 的 `dsh` bin。
- [dsh-cmdline](../cmdline/README.zh.md)——各 bin 使用的启动器到应用命令行交接。
- [Profile 组合包](../../bundle/README.zh.md)——组合进 Web 后端 的可安装 patch 层。
