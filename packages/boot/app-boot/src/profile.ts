 ...
 * layers (additional patch files and deployment settings).
 *
 * Module resolution is two-anchor by construction: a bundle name resolves
 * first from the backend installation, then from the
 ...
 * @param name - the profile directory name.
...
/** Initial bundles for a profile with no shipped template. */
export const DEFAULT_PROFILE_BUNDLES: readonly string[] = ['@deepseek-ai/dsh-base']

/** Custom profiles reload user patch files while the backend remains active. */
...
/**
 * Add the shipped reload default to an exact current bundle tuple. A changed value
...
function normalizeShippedProfile(name: string, dir: string, manifest: ProfileManifest): ProfileManifest {
  ...
  if (!needsReloadDefault) return manifest
...
export function resolveBundleDir(
  ...
  throw new Error(
    ...
    + `install its dependency in ${profileDir}`,
  )
}
...
export function loadProfile(
  ...
  if (!existsSync(join(dir, 'package.json'))) {
    ...
    if (template === undefined) {
      throw new Error(
        `${binName}: profile ${JSON.stringify(name)} does not exist; create its package.json and cordis.patch.yml before loading`,
      )
    }
