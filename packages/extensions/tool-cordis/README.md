...
The CLI example [`apps/server/config/examples/cordis/cordis.yml`](../../../apps/server/config/examples/cordis/cordis.yml) composes both. A package with a browser half additionally needs the browser runner and the UI package in the client composition; a host-only package needs none of them.
