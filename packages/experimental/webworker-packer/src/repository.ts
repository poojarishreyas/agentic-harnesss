...
const SERVER_PACKAGE = 'apps/server'

/** Composition entry point: the `dsh` CLI, run from source. */
const SERVER_ENTRY = `${SERVER_PACKAGE}/src/index.ts`
...
export function composeProfile(repoRoot: string, profile: string): string {
  if (profile !== 'web') throw new Error('vfs image: only the Web application is available')
  const home = mkdtempSync(join(tmpdir(), 'dsh-pack-home-'))
  try {
    return execFileSync(
      process.execPath,
      ['--import', 'tsx/esm', join(repoRoot, SERVER_ENTRY), '--dump-default-config'],
...
export function configTrees(repoRoot: string): ConfigTree[] {
  const packageDir = join(repoRoot, SERVER_PACKAGE)
  ...
  if (!Array.isArray(declared)) {
    throw new Error(`vfs image: ${SERVER_PACKAGE} dsh.configTrees must be an array`)
  }
  const mounts = new Set<string>()
  return declared.map((entry, index) => {
    const tree = entry as Partial<ConfigTreeDeclaration> | null
    const at = `${SERVER_PACKAGE} dsh.configTrees[${String(index)}]`
