...
运行 `pnpm start`，界面会在你的默认浏览器中打开，即可与 agent（智能体）交互式聊天。你会获得会话视图、模型与设置管理以及会话历史，背后与其他表层相同的模型访问、工具与安全默认值。该命令会打印带 token 的启动 URL；浏览器用该 token 换取签名会话 cookie，再重定向到干净的根 URL。你可以从命令行更改端口、关闭浏览器交接并允许额外主机；有意不支持绑定所有网络接口。需要浏览器中的交互式工作时选择它；`dsh-headless` 是一次性的命令行兄弟表层。
...
Web bundle 会禁用随发行版提供的 DeepSeek 对话 adapter，因此「模型」页面只列出用户设置文档中声明的 provider。请从「设置 → 模型」添加 provider；后续 profile patch 也可将 `llm-deepseek` 行的 `disabled` 设为 `false`，以重新启用原生 adapter。独立的 DeepSeek 网页搜索 provider 不受影响。
...
通过 SSH 启动 `pnpm start` 时，URL 行仍会打印，但不会为你打开浏览器：本地转发地址由 SSH 客户端或编辑器持有。请在自己的机器上打开转发后的 URL；打印出的 URL 指向远端宿主机 loopback 端点。
