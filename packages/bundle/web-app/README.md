...
The Web bundle keeps the bundled DeepSeek chat adapter disabled, so the Models page lists only providers declared in the user settings document. Add a provider from **Settings → Models**; a later profile patch can opt back into the native adapter by setting the `llm-deepseek` row's `disabled` field to `false`. The separate DeepSeek web-search provider is unchanged.
...
When you launch `pnpm start` over SSH, the URL line still prints but the browser is not opened for you: the SSH client or editor owns the local forwarding address. Open the forwarded URL on your machine yourself; the printed URL names the remote host's loopback endpoint.
