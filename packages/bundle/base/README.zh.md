---
description: "Web 后端共享的模型访问、工具、持久会话和安全默认值。"
kind: "package-bundle"
---
...
`dsh-base` 为 Web 后端提供模型访问、工具、持久会话和工作区安全默认值。Web profile 首先包含该 bundle；后续 bundle 和 profile 补丁可定制其默认值。
...
后端首次启动时，Web profile 自动包含核心。

### Web profile 组合

Web profile 依次包含 `@deepseek-ai/dsh-base` 和 `@deepseek-ai/dsh-web-app`。使用 `pnpm start` 启动后端；按照[后端参考](../../../apps/server/reference/README.zh.md)定制 `cordis.patch.yml` 或 profile manifest。
...
patch 会替换目标行的整个 `config`，而不是合并进它。后续组合包层与用户的 profile `cordis.patch.yml` 按 id 覆盖行，每行最后一次写入生效。按模式取值不同的行不属于这里：每个模式组合包重述自己的完整配置，让任何单一行最多只属于一个组合包层加用户层。完整行集合及其设计依据以行内注释写在 [`cordis.patch.yml`](cordis.patch.yml) 里；[生成的组合图](../../../apps/server/composition.md)负责渲染它。
...
- [生成组合图](../../../apps/server/composition.md)——每个已发布 profile 使用的确切插件集合。
