...
外部插件通过 Web 配置目录的包清单管理；将已安装的 Bundle 加入 `dsh.profile.bundles` 后重启后端。参见[后端参考](../../../apps/server/reference/README.zh.md)。
...
| [`src/transport.ts`](src/transport.ts) | 按行分隔的 JSON-RPC 帧与请求关联 |
