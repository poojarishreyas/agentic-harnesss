...
const SRC_BIN = '/repo/apps/server/src/index.ts'
...
describe('resolveExampleLaunch', () => {
  ...
  it('lib mode: plain node on the derived lib bin, no tsx and no paths env', () => {
    ...
    expect(args).toContain('/repo/apps/server/lib/index.js')
  ...
  it('lib mode: rewrites only the last /src/ segment', () => {
    const { args } = resolveExampleLaunch({
      srcBin: '/repo/src/apps/server/src/index.ts',
      mode: 'lib',
    })
    expect(args).toContain('/repo/src/apps/server/lib/index.js')
  })
