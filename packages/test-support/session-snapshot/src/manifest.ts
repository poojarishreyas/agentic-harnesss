...
/** Browser or internal base-bundle controller for a recorded-session scenario. */
export type SnapshotProfile = 'web' | 'test-base'
...
const PROFILES = new Set<SnapshotProfile>(['web', 'test-base'])
...
export function parseSnapshotManifest(source: string, path = 'snapshot.yml'): SnapshotManifest {
  ...
  try {
    ...
    if (typeof root.profile !== 'string' || !PROFILES.has(root.profile as SnapshotProfile)) {
      throw new Error('manifest.profile must be web or test-base')
    }
