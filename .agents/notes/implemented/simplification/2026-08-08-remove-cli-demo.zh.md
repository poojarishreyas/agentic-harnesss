...
`apps/server/tests/profiles/headless` 加载已交付的 `base` 与 `headless` 组合包层，然后只对回放或模拟提供方、隔离持久化及被测行为应用窄 overlay。支持层的 `@deepseek-ai/dsh-loader-smoke` 包负责交付 profile 组装 helper、共享的直接 agent 轮次 helper 与仅限测试的 driver；该 driver 将规范事件渲染为 JSONL，但不定义受支持的产品输出格式。
