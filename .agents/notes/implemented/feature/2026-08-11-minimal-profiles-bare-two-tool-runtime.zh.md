...
Web minimal preset 对外提供持久 `bash` 与 `str_replace_editor`，不挂载上下文压缩提供方，为新会话抑制 system-prompt 的 runtime-context 贡献，并让编辑器使用 agent 本地的 `@deepseek-ai/dsh-fs-local` 提供方。其他 Web agent 仍使用宿主文件系统提供方。固定 persona 由 [minimal preset 组合决策](../bug-fix/2026-08-10-minimal-preset-owns-rl-composition.zh.md)拥有。Web 宿主保留沙箱与批准服务。
...
Minimal 会话不会摘要或替换较早历史，也不会添加 runtime-context 快照；调用方必须让会话轮次保持在所选模型的上下文容量内，且不得依赖模型可见的常驻沙箱或批准策略说明。编辑器可以访问运行时进程可见的任何绝对路径，且不受持久 shell 沙箱策略影响。
