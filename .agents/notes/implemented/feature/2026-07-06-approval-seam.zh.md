...
单独挂载该条目只提供机制；没有应答器时，请求返回 `unavailable`，工具调用拒绝执行。Web 后端组合应答器，向用户呈现请求并返回一次性决定。`policy: never` 自动拒绝请求，当前策略写入运行时上下文；插件加载时验证策略值。
...
应答者通过[仅面向自动化的 ACP Agent Note](../../archived/simplification/2026-07-23-acp-automation-only-protocol.md)描述的桥精确 agent 归属检查进行路由，保留了[归档记录：多会话 Agent Note](../../archived/feature/2026-06-14-acp-multi-session.md) 要求的每会话权限归属。
...
- [仅面向自动化的 ACP Agent Note](../../archived/simplification/2026-07-23-acp-automation-only-protocol.md)——应答者路由时对会话映射执行的精确 agent 归属检查；[归档记录：多会话 Agent Note](../../archived/feature/2026-06-14-acp-multi-session.md)——本设计实现的每会话权限归属阻塞项。
