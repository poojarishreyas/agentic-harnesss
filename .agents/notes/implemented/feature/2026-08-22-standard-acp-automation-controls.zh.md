...
> 本说明仅取代 [ACP 作为纯自动化协议](../../archived/simplification/2026-07-23-acp-automation-only-protocol.md) 中仅支持提示词的协议清单。该决策关于禁止 ACP 成为第二套产品 UI 的规定仍具权威性。

> [Web-only 应用决策](../simplification/2026-09-04-web-only-application.zh.md)取代本笔记涉及的公开 SDK、headless 与 ACP 服务器入口。共享运行时机制、外部 ACP 子代理消费方及测试设计的独立理由仍然适用。
