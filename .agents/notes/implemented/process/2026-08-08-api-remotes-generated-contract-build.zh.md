...
Host 与 Client 两次 tsdown 都接收 `vendor/*`、`packages/*/*` 和 `apps/server` 这组完整 workspace。根配置不扫描 `lib/types/client/index.js`，不维护 package 分类表，也不使用 tsdown filter；包内配置根据 `DSH_BUILD_FACE` 返回本阶段入口。
