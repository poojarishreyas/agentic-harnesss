...
`dsh` 启动器硬编码了自己的组合：`base.cordis.yml` + `web.cordis.yml` 随 `apps/server` 一起交付，三种各自定制的入口模式（`--config`、`web`、`-p`）各带一套层栈，外加一个全局的个人 overlay（`$DSH_HOME/config.yaml`）。想把树外插件（一个 TUI、一个提供方扩展包）装进已交付的表层，只能修改仓库；第三方包也没有任何位置可以贡献默认组合。
...
Web 后端的固定 `web` profile 依次应用 `@deepseek-ai/dsh-base` 与 `@deepseek-ai/dsh-web-app`。外部依赖由包管理器安装，组合包成员通过 profile 清单显式声明；后端不提供 profile 选择或插件安装命令。[Web-only 应用决策](../simplification/2026-09-04-web-only-application.zh.md)取代其他产品入口，保留本笔记的有序分层与模块解析决策。
...
- 外部提供方与配置组合包可在 Web profile 中组合，无需把部署特定行加入仓库。
- `apps/server` 负责后端参数解析与 profile 启动；插件安装由部署的包管理器负责。
