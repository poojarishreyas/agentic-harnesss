...
The Web backend uses the fixed `web` profile with `@deepseek-ai/dsh-base` followed by `@deepseek-ai/dsh-web-app`. A package manager installs external dependencies, and the profile manifest explicitly declares bundle membership; the backend offers no profile selector or plugin installation command. The [Web-only application decision](../simplification/2026-09-04-web-only-application.md) supersedes the other product entries while retaining this note’s ordered layering and module-resolution decisions.
...
- External providers and configuration bundles compose within the Web profile without adding deployment-specific rows to the repository.
- `apps/server` owns backend argument parsing and profile startup; the deployment’s package manager owns dependency installation.
