...
const profiles = ['session', 'web'] as const
...
async function scenarios(): Promise<Scenario[]> {
  ...
  for (const profile of profiles) {
    ...
    for (const entry of await readdir(root, { withFileTypes: true })) {
      ...
      expect(manifest.profile, `${profile}/${entry.name}: profile`).toBe(profile === 'session' ? 'test-base' : profile)
...
it('keeps every recorded session owned, pinned, redacted, and header-scrubbed', async () => {
  ...
  for (const scenario of all) {
    ...
    expect(existsSync(join(dir, 'input.json')), `${key}: unexpected protocol input`).toBe(false)
    expect(existsSync(join(dir, 'stdout.expected.jsonl')), `${key}: unexpected stdio transcript`).toBe(false)
