...
describe('cordisConfigFiles', () => {
  it('finds Loader YAML without treating translation records as configs', () => {
    ...
    roots.push(root)
    for (const directory of ['.claude', 'apps/server/config/examples', 'docs', 'node_modules/pkg', 'vendor/pkg']) {
      mkdirSync(join(root, directory), { recursive: true })
    }
    for (const file of [
      '.claude/hidden.cordis.yml',
      'docs/cordis-primer.i18n.yaml',
      'apps/server/config/examples/agent.cordis.yaml',
      'apps/server/config/examples/web.cordis.yml',
    ...
    expect(cordisConfigFiles(root)).toEqual([
      join('apps', 'server', 'config', 'examples', 'agent.cordis.yaml'),
      join('apps', 'server', 'config', 'examples', 'web.cordis.yml'),
    ])
  })
})
