...
// resolve from apps/server.
const appOverlayFiles = new Set([
  ...globSync('apps/server/config/examples/**/*.yml', { cwd: root }),
])
...
function validateAppResolution(): string[] {
  ...
  // App overlays (and any config left under apps/server/config) resolve from the
  // dsh app's own dependency surface — the profile module fallback mirrors it.
  const appManifest = readManifest('apps/server/package.json')
  ...
  const shipped = new Set(globSync('*.cordis.yml', { cwd: resolve(root, 'apps/server/config') })
    .map(file => `apps/server/config/${file}`))
  ...
  violations.push(...missingPluginDependencies(
    appReferences,
    appDependencies,
    'apps/server/package.json dependencies or a bundle manifest',
  ))
  const appTestReferences = pluginReferences.filter(reference => reference.file.startsWith('apps/server/tests/'))
  violations.push(...missingPluginDependencies(
    appTestReferences,
    { ...appManifest.dependencies, ...appManifest.devDependencies },
    'apps/server/package.json dependencies or devDependencies',
  ))
