/** Recorded-session replay through an internal base-bundle test driver. */
...
const baseDriver = join(repoRoot, 'packages/test-support/loader-smoke/tests/fixtures/base-driver.ts')
...
function snapshotSpillRoot(fixtureFile: string): string {
  const key = createHash('sha256').update(basename(dirname(fixtureFile))).digest('hex').slice(0, 9)
  const root = process.platform === 'win32' ? '/t' : '/tmp'
...
async function fixtureSessions(scenario: BaseScenario): Promise<string[]> {
...
function modelFromSession(log: string): { provider: string; model: string } {
  ...
  throw new Error('base snapshot session has no request model')
}

async function seedWorkspace(scenario: BaseScenario, cwd: string): Promise<void> {
...
async function collectScenarios(): Promise<BaseScenario[]> {
  ...
  for (const entry of await readdir(snapshotsRoot, { withFileTypes: true })) {
    ...
    if (manifest.profile !== 'test-base' || manifest.composition === undefined) continue
    if (manifest.recording === undefined || manifest.header === undefined) {
      throw new Error(`${entry.name}: a base corpus manifest needs recording and header metadata`)
    }
    scenarios.push({
      name: entry.name,
      dir,
...
const compositionOwners = new Map<string, BaseScenario>()
const headerPins = new Map<string, BaseScenario>()
for (const scenario of scenarios) {
  const { composition, header } = scenario.manifest
  if (existsSync(join(scenario.dir, 'cordis.yml'))) {
    if (compositionOwners.has(composition)) throw new Error(`base composition ${composition} has multiple patch owners`)
    compositionOwners.set(composition, scenario)
  }
  if (header.pin === true) {
    const key = `${composition}/${header.class}`
    if (headerPins.has(key)) throw new Error(`base header class ${key} has multiple pins`)
    headerPins.set(key, scenario)
  }
}

function ownerOf(scenario: BaseScenario): BaseScenario {
...
function pinOf(scenario: BaseScenario): BaseScenario {
  const { composition, header } = scenario.manifest
...
async function verifyHeaders(scenario: BaseScenario, actualLogs: readonly SessionLog[], ctx: NormalizeContext): Promise<void> {
  ...
  if (promptOwner === undefined || schemaOwner === undefined) {
    throw new Error(`${scenario.name}: header sidecar source is not a base scenario`)
  }
...
describe('base recorded-session snapshots', () => {
  ...
  for (const scenario of scenarios) {
    ...
    scenarioTest(`${mode}s ${scenario.name} through the internal base driver`, async () => {
      ...
      if (baseComposition === undefined) throw new Error('base corpus has no default composition')
      ...
      const resultRow = JSON.parse(result.stdout.trimEnd().split('\n').at(-1) ?? '{}') as { output?: unknown }
      expect(resultRow.output).toBe(finalTextFromSession(fixtures[0] as string))
