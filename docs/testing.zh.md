...
- **所属位置的预期输出**（`pnpm run test:expected`）：无录制会话往返的无密钥组装 后端/进程预期。驱动使用 `*.expected.e2e.ts`，并与 `tests/expected/` 同属一处；CI 针对构建产物运行。包/脚本预期使用 `test`，浏览器预期使用 `test:web`。
- **快照**（`pnpm run test:snapshot`）：录制的 `session.jsonl` 提供用户输入、模型回放及持久化结果预期值。Web 场景在同一会话旁保留浏览器与 ARIA 证据。变更工作区的场景独立比较完整的 `workspace.expected/` 目录。模型 transcript 变化时使用 `test:snapshot:record`，回放输入仍有效时使用 `test:snapshot:refresh`，并审查所有差异。
...
我们是 DeepSeek，不要吝惜真实 API 测试。无密钥测试只能证明底层通路；只有带密钥运行才能证明 agent（智能体）能对接真实模型正常工作。覆盖文件写入提示词、包含多个轮次的对话、工具使用和流中取消。价值最高的是**冒烟测试**：启动 Web 后端、发送一条提示词，并检查外部世界；它们能捕获「单元测试全绿、产品却坏了」这一类 mock 无法发现的问题（[事故复盘 0001](postmortem/0001-acp-default-export-drops-inject.zh.md)）。自动跳过让无密钥 CI 和无密钥贡献者不受阻塞；它不是成本信号。Profile 级集成测试位于 `apps/server/tests/profiles/`；包专属组合留在对应包的测试目录中。

## 优先使用真实实现而非 mock

只 mock 开销高或不确定的边界（LLM（大语言模型）适配器、网络、时钟）；下游一切保持真实。手写替身只能证明桥接层在搬运字节，不能证明交付的工具行为符合断言。
...
- “真实入口路径”包括由普通 Node 运行的后端构建入口 `apps/server/lib/index.js`、工作线程入口及跨 bundle 共享的模块。构建产物 smoke 测试应验证启动、模块解析与关闭行为，并断言缺失配置会非零退出。
...
每项非平凡的模型可见、协议可见或用户可见变更，都在同一 PR 中添加或更新无密钥录制会话场景。Web 录制位于 `snapshots/web/`；`snapshots/session/` 下的核心智能体与工具录制使用内部 `test-base` 驱动。不由录制会话驱动的预期输出保留在所属应用、包或脚本的 `tests/expected/` 下。[`dsh-session-snapshot`](../packages/test-support/session-snapshot/README.zh.md) 定义共享存储规则。智能体循环、会话生命周期与 `SessionEventMap` 变更必须覆盖浏览器使用的后端事件及持久化状态。
