...
sequenceDiagram
  ...
  participant Session
  participant UI as Web listener
  User->>Agent: followup(content)
  Agent-->>UI: <code>agent/inbox/spliced</code>
  Agent-->>UI: <code>agent/inbox/inserted</code> { message }
  Agent->>Driver: queued work wakes driver
  Driver-->>UI: <code>agent/status</code> running
  Driver->>Session: <code>turn/start</code>
  Note over Agent,Driver: claim pending next-step input plus one queued prompt
  Driver-->>UI: <code>agent/inbox/spliced</code> pure deletion
  Driver-->>UI: <code>agent/inbox/claimed</code> { message, turn } per message
  ...
  Session-->>UI: <code>session/event</code> <code>assistant/chunk</code>*
  ...
  Driver-->>UI: <code>agent/status</code> idle
```
...
需要可回放 transcript（文本记录）数据的 Web 消费方应当消费 `session/event`；`agent/*` 是用于队列与状态、提示词拦截、请求构造、steering、继续执行和错误处理的实时协调接口。
