...
The Web backend is a Cordis plugin tree composed from ordered configuration layers. Its fixed `web` profile lives in the Harness home and records the bundles, external plugin dependencies, and user-owned `cordis.patch.yml`.

[`dsh-base`](../packages/bundle/base/README.md) supplies agents, models, tools, persistence, sandbox and approval policy, settings, credentials, and telemetry. [`dsh-web-app`](../packages/bundle/web-app/README.md) adds the HTTP backend and browser application.

Each bundle declares `dsh.bundle.patch` in its package manifest. Layers apply to an empty entry list in this order: the profile's bundles, the profile patch, the home-level patch, and ordered `--patch` overlays. A patch targets a row by id and replaces its whole config or inserts rows. The Web profile supports live patch reload.

Composition mechanics are in [app-boot](../packages/boot/app-boot/README.md#profiles); configuration fields are in the generated [config catalog](config-catalog.md).

## Application launch

[`apps/server`](../apps/server/README.md) owns the Web backend entry point. `pnpm start` runs its source entry with the tsx ESM hook; the built entry is `node apps/server/lib/index.js`. The backend boots the fixed `web` profile and preserves Harness home, plugin layers, configuration patches, and shutdown handling.

The browser communicates with this backend through the Web API. Agent execution, shell tools, approvals, and session persistence remain backend services. The repository provides no standalone CLI task runner, public TypeScript or Python SDK, or ACP server application.

Build and test executables are development infrastructure. [`verify-application-entrypoints`](../scripts/verify-application-entrypoints.ts) checks the permitted application entries.
