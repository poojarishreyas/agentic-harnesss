...
The Web backend composes core and browser application layers through `packages/bundle/*/cordis.patch.yml`. User overlays live under `apps/server/config/examples/`; backend composition tests live under `apps/server/tests/`, while package-specific Loader compositions stay with their package tests.
